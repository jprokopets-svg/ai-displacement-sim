    ...
    <>
      ...
      {selected && (
        <div style={resultCardStyle}>
          <div style={{ fontSize: 13, color: '#c6cfdf', marginBottom: 4 }}>
          ...
          <div style={{ fontSize: 11, color: '#aab7cb', marginTop: 4 }}>
            composite AI displacement score · p{Math.round(selected.exposure_percentile)}
          </div>
          <div style={{ fontSize: 12, color: '#c6cfdf', marginTop: 4 }}>
          ...
          {topOccs.length > 0 && (
            <div style={{ marginTop: 14, borderTop: '1px solid #1f2942', paddingTop: 10 }}>
              <div style={{ fontSize: 11, color: '#aab7cb', marginBottom: 6, textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>
              ...
              {topOccs.slice(0, 3).map((o, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0', fontSize: 13, borderBottom: '1px solid #1f2942' }}>
                  <span style={{ color: '#c6cfdf', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{o.occupation_title}</span>
...
function JobChecker() {
  ...
  return (
    <>
        ...
        <div style={{ marginTop: 8 }}>
          {results.map(o => (
            <div key={o.soc_code} onClick={() => setSelected(o)} style={resultRowStyle}>
              ...
              <span style={{ fontFamily: 'ui-monospace, monospace', fontSize: 12, color: '#aab7cb' }}>{o.soc_code}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {selected && score != null && (
        <div style={resultCardStyle}>
          <div style={{ fontSize: 13, color: '#c6cfdf', marginBottom: 4 }}>{selected.occupation_title}</div>
          <div style={{
            fontFamily: 'ui-monospace, monospace', fontSize: 40, fontWeight: 600,
          ...
          <div style={{ marginTop: 14, borderTop: '1px solid #1f2942', paddingTop: 10 }}>
            ...
            {[
              ...
              <div key={p.year} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 0' }}>
                <span style={{ fontSize: 13, color: '#c6cfdf' }}>{p.year}</span>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section style={sectionStyle}>
      <div style={{ fontSize: 10, letterSpacing: '0.1em', color: '#aab7cb', textTransform: 'uppercase' as const, fontWeight: 600, marginBottom: 10 }}>
        {title}
      </div>
      {children}
    </section>
...
const subStyle: React.CSSProperties = {
  fontSize: 13, color: '#aab7cb', margin: 0, lineHeight: 1.5,
}

const bodyStyle: React.CSSProperties = {
  fontSize: 14, color: '#c6cfdf', margin: '0 0 12px', lineHeight: 1.5,
}
...
const footerStyle: React.CSSProperties = {
  ...
  fontSize: 13, color: '#aab7cb', textAlign: 'center',
}
